...
public class GetMyEventsApi extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		List<Sale> sas = OfyService.ofy().load().type(Sale.class).list();
		for (Sale sa : sas) {
			if (sa.getUserId().equals(id)) {
				myEvents.sales.add(sa);
			}
		}	
		
		List<Sports> sps = OfyService.ofy().load().type(Sports.class).list();
		for (Sports sp : sps) {
			if (sp.getInitiatorId().equals(id)) {
